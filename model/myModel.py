...
class Point_M2AE_SEG(nn.Module):
    def __init__(self, is_learned, num_classes=1, text_prompts=None):
        super().__init__()
...
class StudentNetworks(nn.Module):
    def __init__(self, args, num_classes, text_prompts=None, emb=681, freeze = True):
        super().__init__()
    ...
    def forward(self, x, return_pts_feat=False, return_mlp_feat=False):
        feature = self.extractor(x) #[BS, e=681, n_pts=2048]
        
        x, mlp_feat = self.distillation_head(feature.permute(0,2,1))
        
        if return_pts_feat:
            return x, feature
        elif return_mlp_feat:     
            return x, mlp_feat
        else:
            return x
        
class Point_M2AE_ReductionD(nn.Module):
    def __init__(self, out_dim=96):
        super().__init__()
        self.emb=681
        
        self.pointm2ae = Point_M2AE_SEG(is_learned=False)
        state_dict = torch.load("/home/huyy23/project/PartDistill-main/checkpoints/point_m2ae_pre-train.pth")
        self.pointm2ae.load_state_dict(state_dict['base_model'], strict=False)
        for param in self.pointm2ae.parameters():
            param.requires_grad = False
            
        self.out_dim = self.emb
        self.fc1 = nn.Linear(self.emb, self.emb)
        self.fc2 = nn.Linear(self.emb, self.emb)
        self.fc3 = nn.Linear(self.emb, self.emb)
        self.relu = nn.ReLU()
    ...
    def forward(self, x):
        ...
        f = self.relu(self.fc1(f.permute(0,2,1)))
        f = self.relu(self.fc2(f))
        f = self.relu(self.fc3(f))
...
class MyModel(nn.ModuleList):
    def __init__(self, num_labels):
        super().__init__()
        self.encoder = Point_M2AE_ReductionD()
        self.classifier = nn.Linear(self.encoder.out_dim, num_labels)
    def forward(self, x):
        x = self.encoder(x)
        ret_x = self.classifier(x)
        return ret_x

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--backbone_path", type=str, default="/home/huyy23/project/PartDistill-main/checkpoints/point_m2ae_pre-train.pth")
    args = parser.parse_args()
    model = StudentNetworks(args,4,freeze=True)
    pass